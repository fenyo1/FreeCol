...
\item The checkbox \Option{Default: Colonies make new colonists} allows you to change
  whether to make a new colonist when the food reaches 200 in a colony.
  This is a global default value, which applies to all colonies.
  In a colony this is overridden by the Per colony setting if specified.

\item The checkbox \Option{Default: Breeding Consumes only surplus production}:
  When NOT checked: Breedable-type goods (eg. Horses) are breeded without
  respecting the surplus production of the consumed goods (eg. Food).
  This is a global default value, which applies to all colonies.
  In a colony this is overridden by the Per colony setting if specified.

\item The option \Option{Default consume-ratio of surplus production by breeding}:
  The Breedable-type goods (eg. Horses) are breeded with respecting the surplus production
  of the consumed goods (eg. Food), using only this percent of the consumed goods.
  This is a global default value, which applies to all colonies.
  In a colony this is overridden by the Per colony setting if specified.
